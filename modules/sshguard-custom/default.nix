...
let
  cfg = config.services.sshguard-custom;
...
in {
  ...
  options = {
    ...
    services.sshguard-custom = {
      ...
      ipv6_subnet = mkOption {
        default = 128;
        example = 64;
        type = types.int;
        description = lib.mdDoc ''
            IPv6 subnet size to block
          '';
      };

      ipv4_subnet = mkOption {
        default = 32;
        example = 24;
        type = types.int;
        description = lib.mdDoc ''
            IPv4 subnet size to block
          '';
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."sshguard.conf".source = pkgs.writeText "sshguard.conf" ''
      BACKEND="${pkgs.sshguard}/libexec/${backend}"
      LOGREADER="LANG=C ${config.systemd.package}/bin/journalctl ${args}"

      IPV6_SUBNET=${toString cfg.ipv6_subnet}
      IPV4_SUBNET=${toString cfg.ipv4_subnet}
    '';
    ...
    systemd.services.sshguard = {
      ...
      restartTriggers = [ config.environment.etc."sshguard.conf".target ];
